  ...
  void loadBalance() {
    if (AppSingleton.userInfoModel?.walletAddress != null ||
      ...
      TokenTx.getBalances(AppSingleton.userInfoModel?.walletAddress ?? "")
        ...
        value.forEach((element) {
          if (element.symbol == WalletType.RSGT.name) {
            log("rsgt ---->>>${WalletType.RSGT.name}");
            setState(() {
              assets[0].name = WalletType.USDT.name;
              assets[0].amount = element.balance;
            });
          } else if (element.symbol == WalletType.USDT.name) {
            setState(() {
              assets[2].name = WalletType.USDT.name;
              assets[2].amount = element.balance;
            });
          }
        });
      });
      Tx.getBalance(AppSingleton.userInfoModel?.walletAddress ?? "")
          .then((value) {
        log("bnb balance --->>>$value");
        setState(() {
          assets[1].amount = value;
        });
      });
          ...
          : SmartRefresher(
              ...
              child: Stack(
                children: [
                  Container(
                    margin:
                        const EdgeInsets.only(top: 390, left: 15, right: 15),
...
class ListItem {
  ...
  ListItem(this.coin, this.name, this.amount, this.icon, {this.type});

  @override
  String toString() {
    return 'ListItem{coin: $coin, name: $name, amount: $amount, icon: $icon, type: $type}';
  }
}
